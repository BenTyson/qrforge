...
const PLATFORM_CONFIG: Record<string, { name: string; baseUrl: string; color: string; hoverColor: string; icon: ReactNode }> = {
  twitter: {
    name: 'X (Twitter)',
    ...
    >
      ...
      <div className="max-w-md mx-auto relative z-10">
        ...
        >
          ...
          {content.bio && (
            <p className="text-slate-400 mt-2 max-w-sm mx-auto text-sm leading-relaxed">{content.bio}</p>
          )}
        </div>
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-white/70 truncate">@{link.handle}</p>
                </div>
                <svg
                  className="w-5 h-5 text-white/50 transition-all duration-300 group-hover:text-white/80 group-hover:translate-x-1"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2"
        ...
        {/* Powered by */}
        <p
          className="mt-12 text-center text-sm text-slate-600 animate-fade-in"
          style={{ animationDelay: `${content.links.length * 80 + 200}ms`, animationFillMode: 'backwards' }}
        >
          Powered by{' '}
