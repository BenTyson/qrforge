...
const SOCIAL_ICONS: Record<string, { icon: ReactNode; color: string }> = {
  twitter: {
    icon: (
...
export function LinksPreview({ content, className }: LinksPreviewProps) {
  const accentColor = content.accentColor || '#14b8a6';
  const title = content.title || 'Your Links';
    ...
    <div className={cn('flex flex-col items-center', className)}>
      ...
      <div className="relative w-[280px] h-[560px] rounded-[3rem] bg-slate-900 p-2 shadow-2xl border-4 border-slate-700">
        ...
        <div
          ...
          style={{
            background: `linear-gradient(180deg, ${accentColor}20 0%, #0f172a 40%, #0f172a 100%)`,
          }}
        >
          {/* Content */}
          <div className="p-5 pt-10">
            ...
            <div className="space-y-2.5">
              {links.length === 0 ? (
                <div className="text-center py-6">
                  <p className="text-xs text-slate-500">Add links to see preview</p>
                </div>
              ) : (
                links.map((link, index) => (
                  <div
                    key={index}
                    className="w-full py-3 px-4 rounded-xl text-center text-sm font-medium text-white transition-all"
                    style={{
              ...
              <div className="flex justify-center gap-3 mt-6">
                {socialLinks.map((social, index) => {
                  ...
                  return (
                    <div
                      key={index}
                      className="w-9 h-9 rounded-full bg-slate-800/80 flex items-center justify-center border border-slate-700/50"
            ...
            <div className="mt-8 text-center">
              <p className="text-[9px] text-slate-500">Powered by QRWolf</p>
            </div>
          </div>
        </div>
      </div>
