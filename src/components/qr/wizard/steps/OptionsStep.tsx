...
import { getTimezoneOptions } from '@/lib/scheduling/utils';
import { SchedulePreview } from '@/components/qr/schedule/SchedulePreview';
import type { ScheduleMode } from '@/components/qr/studio/hooks/useProOptions';
                    ...
                    <div className="space-y-3">
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) => actions.setScheduleTimezone(e.target.value)}
                          className="w-full mt-1 px-3 py-2 bg-slate-900 border border-slate-600 rounded-md text-sm text-white focus:ring-primary focus:border-primary"
                        >
                          {(() => {
                      ...
                      <div>
                        ...
                        <div className="flex gap-1 mt-1 p-1 bg-slate-800 rounded-lg">
                          {([
                            { value: 'once' as ScheduleMode, label: 'One-time' },
                            { value: 'daily' as ScheduleMode, label: 'Daily' },
                            { value: 'weekly' as ScheduleMode, label: 'Weekly' },
                          ]).map(tab => (
                            <button
                              key={tab.value}
                              onClick={() => actions.setScheduleMode(tab.value)}
                              className={cn(
                      ...
                      {(scheduleMode === 'daily' || scheduleMode === 'weekly') && (
                        <div className="space-y-3">
                          <p className="text-xs text-slate-500">Set the daily time window when this QR code is scannable</p>
                          <div className="grid grid-cols-2 gap-2">
                            <div>
                              <Label className="text-xs text-slate-400">Start time (daily)</Label>
                              <Input
                                type="time"
                                value={recurringStartTime}
                                onChange={(e) => actions.setRecurringStartTime(e.target.value)}
                                className="bg-slate-900 border-slate-600 text-sm"
                              />
                            </div>
                            <div>
                              <Label className="text-xs text-slate-400">End time (daily)</Label>
                              <Input
                                type="time"
                                value={recurringEndTime}
                                onChange={(e) => actions.setRecurringEndTime(e.target.value)}
                                className="bg-slate-900 border-slate-600 text-sm"
                              />
                            </div>
                          </div>
                              ...
                              <div className="flex gap-1 mt-1">
                                {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((day, idx) => (
                                  <button
                                    key={day}
                                    onClick={() => {
                                      const days = recurringDaysOfWeek.includes(idx)
                                        ? recurringDaysOfWeek.filter(d => d !== idx)
                                        : [...recurringDaysOfWeek, idx].sort();
                                      actions.setRecurringDaysOfWeek(days);
                                    }}
                                    className={cn(
                                      'flex-1 py-1.5 text-[10px] font-medium rounded-md transition-all border',
                      ...
                      {/* Schedule preview */}
                      <SchedulePreview
                        activeFrom={scheduleMode === 'once' ? activeFrom : null}
                        activeUntil={scheduleMode === 'once' ? activeUntil : null}
                        timezone={scheduleTimezone}
                        rule={scheduleMode !== 'once' ? {
                          type: scheduleMode,
                          startTime: recurringStartTime || '09:00',
                          endTime: recurringEndTime || '17:00',
                          ...(scheduleMode === 'weekly' ? { daysOfWeek: recurringDaysOfWeek } : {}),
                        } : null}
                      />

                      {/* What happens outside scheduled times */}
                      <div className="p-3 rounded-lg bg-slate-800/80 border border-slate-700/50">
                        <div className="flex gap-2">
                          ...
                          <div className="text-xs text-slate-400 space-y-1">
                            <p className="font-medium text-slate-300">What happens outside scheduled times?</p>
                            {scheduleMode === 'once' ? (
                              <p>
                                Before the start time, scanners see a &quot;Not Yet Active&quot; page.
                                After the end time, they see a &quot;No Longer Active&quot; page.
                                Both pages link back to QRWolf.
                              </p>
                            ) : (
                              <p>
                                Anyone who scans this QR code outside the active hours
                                {scheduleMode === 'weekly' ? ' or on an inactive day' : ''} will
                                see a &quot;Currently Inactive&quot; page explaining the code is on
                                a recurring schedule. They won&apos;t be redirected to your destination
      ...
      {(expiresAt || (passwordEnabled && password) || (scheduledEnabled && (activeFrom || activeUntil)) || (abTestEnabled && abVariantBUrl)) && (
        <div className="p-4 rounded-xl bg-primary/5 border border-primary/20">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {scheduledEnabled && (activeFrom || activeUntil || scheduleMode !== 'once') && (
              <span className="text-xs bg-primary/20 text-primary px-2 py-1 rounded-full">
                {scheduleMode === 'once' ? 'Scheduled' : scheduleMode === 'daily' ? 'Daily Schedule' : 'Weekly Schedule'}
              </span>
            )}
