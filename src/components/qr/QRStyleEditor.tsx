...
export function QRStyleEditor({ style, onChange, canAccessPro = false }: QRStyleEditorProps) {
  const [isExpanded, setIsExpanded] = useState(false);
  const [activeSection, setActiveSection] = useState<'colors' | 'pattern' | 'frame'>('colors');

  const updateStyle = (updates: Partial<QRStyleOptions>) => {
    onChange({ ...style, ...updates });
  };

  // Check if any advanced features are in use
  const hasAdvancedFeatures =
    (style.moduleShape && style.moduleShape !== 'square') ||
    (style.cornerSquareShape && style.cornerSquareShape !== 'square') ||
    (style.cornerDotShape && style.cornerDotShape !== 'square') ||
    style.frame?.enabled;
      ...
      >
        <div className="px-6 pb-6 space-y-6">
          ...
          <div className="flex border-b border-border/50">
            {(['colors', 'pattern', 'frame'] as const).map((section) => (
              <button
                key={section}
          ...
          {activeSection === 'pattern' && (
            <div className="space-y-6">
              <PatternSelector
                ...
                onChange={(shape) => updateStyle({ moduleShape: shape })}
                canAccessPro={canAccessPro}
              />
              <EyeStyleSelector
                cornerSquareValue={style.cornerSquareShape}
                cornerDotValue={style.cornerDotShape}
                onCornerSquareChange={(shape) => updateStyle({ cornerSquareShape: shape })}
                onCornerDotChange={(shape) => updateStyle({ cornerDotShape: shape })}
                canAccessPro={canAccessPro}
              />
            </div>
          )}
